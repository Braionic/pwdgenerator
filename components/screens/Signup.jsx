...
const Signup = ({ navigation }) => {
  ...
  let newUserSchema = object({
    ...
    password: string().required('password is required').min(6, 'must be at least 6 characters long'),
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={{ flex: 1, alignItems: "center", backgroundColor: "white" }}>
        ...
        />
{error &&<View style={{backgroundColor: 'red', padding: 5, marginTop: 20, borderRadius: 5}}><Text style={{color: 'white', textAlign: 'center', fontSize: 18}}>{error}</Text></View>}

        <KeyboardAvoidingView>
        <Formik
     initialValues={{ email: '', name: '', password: '', profileImage: '' }}
   ...
   >
    {({ handleChange, handleBlur, handleSubmit, values, errors, touched, isSubmitting }) => (
        <>
          ...
          <Text style={{color: 'red'}}>{errors.profileImage && touched.profileImage && errors.profileImage}</Text>
